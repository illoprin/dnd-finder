...
<nav class="navbar navbar-expand-lg navbar-dark">
  <div class="container">
    ...
    <div class="d-flex align-items-center">
      <a class="navbar-brand fw-bold me-4" href="/">D&D Finder</a>
      <ul class="navbar-nav d-flex flex-row gap-3">
        ...
        <?php if (is_logged_in()): ?>
          <li class="nav-item">
            <a class="nav-link text-warning fw-bold" href="/pages/tinder.php">
    ...
    <div class="d-flex align-items-center gap-3">
      <?php if (is_logged_in()): ?>
        ...
        <div class="dropdown">
          <button class="btn btn-outline-light position-relative" type="button" id="matchesDropdown" data-bs-toggle="dropdown" aria-expanded="false" title="Ваши матчи">
          ...
          <ul class="dropdown-menu dropdown-menu-end shadow mt-2" aria-labelledby="matchesDropdown" style="width: 280px; max-height: 400px; overflow-y: auto;">
            ...
            <?php if (!empty($nav_matches)): ?>
              <?php foreach ($nav_matches as $nav_match): ?>
                ...
                ?>
                <li>
                  <a class="dropdown-item py-2 border-bottom border-light" href="/pages/account.php#matches">
                    <div class="text-truncate fw-bold text-success">@<?= htmlspecialchars($nav_partner) ?></div>
                    <small class="text-white-50 text-wrap d-block" style="font-size: 0.75rem;">
                      Кампания: <?= htmlspecialchars($nav_match['app_title']) ?>
                    </small>
                  </a>
                </li>
              <?php endforeach; ?>
              <li>
                <a class="dropdown-item text-center text-primary small fw-bold py-2" href="/pages/account.php#matches">
                  Смотреть все контакты
                </a>
              </li>
            <?php else: ?>
              <li class="text-center py-3 small">У вас пока нет матчей.<br>Крутите анкеты!</li>
            <?php endif; ?>
          </ul>
        </div>
        ...
        <span class="text-white-50">
          Привет, <a href="/pages/account.php" class="text-white fw-bold text-decoration-none border-bottom border-white border-opacity-25" title="В личный кабинет"><?= htmlspecialchars($_SESSION['user_nickname']) ?></a>
        </span>
